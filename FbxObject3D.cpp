...
ComPtr<ID3D12Device> FbxObject3d::dev;
...
void FbxObject3d::Update()
{
	...
	//ビュープロジェクション行列
	const XMMATRIX& matViewProjection = Camera::Instance()->GetMatProjection();
	//モデルのメッシュトランスフォーム
	const XMMATRIX& modelTransform = model->GetModelTransform();
	//カメラ座標
	const XMFLOAT3& cameraPos = Camera::Instance()->GetEyePos().ConvertXMFLOAT3();
	...
	//ボーン配列
	vector<FbxModel::Bone>& bones = model->GetBones();
	...
	for (int i = 0; i < bones.size(); i++) {
		...
		//XMMATRIXに変換
		FbxModelLoader::ConvertMatrixFromFbx(&matCurrentPose, fbxCurrentPose);
		//合成してスキニング行列に
...
void FbxObject3d::Draw()
{
	...
	//定数バッファビューをセット
	DirectXBase::Instance()->cmdList->SetGraphicsRootConstantBufferView(0, constBuffTransform->GetGPUVirtualAddress());
	//定数バッファビューをセット
	DirectXBase::Instance()->cmdList->SetGraphicsRootConstantBufferView(2, constBuffSkin->GetGPUVirtualAddress());
